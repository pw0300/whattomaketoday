...
import { Dish, SwipeDirection, ImageSize, DietType } from '../types';
            ...
            >
              <div className="bg-ink text-white py-1 px-3 flex justify-between items-center">
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 border border-white flex items-center justify-center rounded-sm ${currentDish.dietType === DietType.Vegetarian ? 'text-green-500' : currentDish.dietType === DietType.Eggetarian ? 'text-yellow-500' : 'text-red-500'}`}>
                    <div className="w-1 h-1 rounded-full bg-current"></div>
                  </div>
                  <span className="font-mono text-[10px] uppercase tracking-widest">{currentDish.dietType.substring(0,3)} / {currentDish.type}</span>
                </div>
              ...
              <div className="flex-1 p-6 flex flex-col items-center text-center relative overflow-hidden">
                 ...
                 <div className="flex gap-2 flex-wrap justify-center z-10">
                    {currentDish.tags.map(tag => (
                        <span key={tag} className="font-mono text-[8px] font-black bg-white border border-ink px-2 py-1 uppercase shadow-sm">
                            {tag}
                        </span>
                    ))}
                 </div>
