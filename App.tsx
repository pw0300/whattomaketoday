...
const App: React.FC = () => {
  ...
  const handleSwipe = (dishId: string, direction: SwipeDirection) => {
    ...
    if (unswiped < 3 && !fetchingMore && userProfile) {
  ...
  const handleOnboardingComplete = async (profile: UserProfile) => {
    ...
    setIsSeeding(true);
    const dishes = await generateNewDishes(8, profile);
  ...
  return (
    <div className="h-screen flex flex-col bg-paper overflow-hidden text-ink">
      ...
      {showReceipt && <Receipt plan={weeklyPlan} missingIngredients={[]} onClose={() => setShowReceipt(false)} onSend={() => {}} />}
    </div>
  );
};
